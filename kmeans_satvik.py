...
def distance(instance1, instance2, dist):
    types = ['euclidean','manhattan','cosine','jaccard']
...
def assignAll(instances, centroids, dist):
...
def kmeans(instances, k, animation=False, initCentroids=None, dist='euclidean', condition='centroid'):
    result = {}
    ...
    #while(centroids != prevCentroids):
        iteration += 1
        clusters = assignAll(instances, centroids, dist)
        if animation:
...
#    clustering = kmeans(dataset, 3, dist='cosine')
